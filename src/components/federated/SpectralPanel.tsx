...
interface SpectralHyperparams {
  numClusters: number;
}

interface SpectralPanelProps {
  value: SpectralHyperparams;
  onChange: (value: SpectralHyperparams) => void;
  collapsed?: boolean;
  onCollapseToggle?: () => void;
}

export const SpectralPanel: React.FC<SpectralPanelProps> = ({ value, onChange, collapsed = false, onCollapseToggle }) => {
  const [local, setLocal] = useState(value);

  const handleChange = (field: keyof SpectralHyperparams, val: any) => {
    ...
    <Card className="mb-4 w-full border-primary/40">
      <CardHeader className="flex flex-row items-center justify-between cursor-pointer select-none" onClick={onCollapseToggle}>
        <CardTitle className="text-base flex items-center gap-2">
          Spectral Clustering – Hyperparamètres
        </CardTitle>
